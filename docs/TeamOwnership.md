...
| `platform` | ~~`terraform import` the CLI-created prod tables~~ **DONE 2026-07-01** (commit `fd3c6db`): all 23 imported via OpenTofu (20 module-declared payments/compliance + 3 newly declared beta tables); plan verified 0 add / 0 destroy, tags-only residual. Remaining choices: apply the standard tags (in-place, safe), and enable PITR + deletion protection on the 3 beta tables (BetaApplications holds applicant PII). |
