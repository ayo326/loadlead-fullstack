      ...
      <div className="max-w-4xl mx-auto p-6">
        {items.length === 0 ? (
          <div className="rounded-xl border bg-card flex flex-col items-center justify-center py-20 text-muted-foreground gap-3">
            ...
            <p className="text-xs">Loads accepted by your fleet drivers will appear here.</p>
          </div>
        ) : (
          <div className="rounded-xl border bg-card divide-y">
            {items.map(({ load, offer }: any) => {
              ...
              return (
                <div key={load.loadId} className="px-5 py-4 flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0 pt-0.5">
                    ...
                    {/* OO doesn't have a dedicated load detail page — show load ID as reference */}
                    <Button size="sm" variant="outline" disabled className="cursor-default text-muted-foreground">
                      #{load.loadId?.slice(-6).toUpperCase()}
                    </Button>
                  </div>
                </div>
              );
