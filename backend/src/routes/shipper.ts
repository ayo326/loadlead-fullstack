...
router.post('/admin-request', asyncHandler(async (req: AuthRequest, res) => {
  const shipper = await ShipperService.getProfileByUserId(req.user!.userId);
...
router.post('/loads/draft', validate(loadValidators.createLoad), asyncHandler(async (req: AuthRequest, res) => {
  const shipper = await ShipperService.getProfileByUserId(req.user!.userId);
